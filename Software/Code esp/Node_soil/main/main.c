...
static const char *TAG = "Soil_NODE";
...
void send_lora_task(void *pvParameters)
{
	...
	char * buf[256]; // Maximum Payload size of SX1276/77/78/79 is 255
	while(1) {
...
void ads111x_task(void *pvParameters)
{
    ...
    {
        ESP_ERROR_CHECK(ads111x_init_desc(&devices[i], ADS111X_ADDR_VCC, 0, CONFIG_EXAMPLE_I2C_MASTER_SDA, CONFIG_EXAMPLE_I2C_MASTER_SCL));
...
void ds18b20_task(void *pvParameter)
{
    ...
    while (1)
    {
        if (ds18x20_measure_and_read(SENSOR_GPIO, SENSOR_ADDR, &temperature) != ESP_OK)
            printf("Could not read from sensor");
        else
            printf("Sensor %08" PRIx32 "%08" PRIx32 ": %.2f°C\n",
